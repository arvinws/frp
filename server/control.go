...
func (ctl *Control) handleNewProxy(m msg.Message) {
	...
	if ctl.clientBanStore != nil {
		effectiveID := ctl.sessionCtx.LoginMsg.ClientID
		if effectiveID == "" {
			effectiveID = ctl.sessionCtx.LoginMsg.RunID
		}
		disabled, record := ctl.clientBanStore.IsDisabled(effectiveID)
		if !disabled {
			remoteIP := ctl.sessionCtx.Conn.RemoteAddr().String()
	...
	if err == nil {
		...
		retContent, pluginErr := ctl.sessionCtx.PluginManager.NewProxy(content)
		if pluginErr == nil {
